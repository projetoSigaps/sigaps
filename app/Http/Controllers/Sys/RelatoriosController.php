...
class RelatoriosController extends Controller
{
	...
	public function horariosDownloadAuto(Request $request)
	{
		...
		if($request->cod_cracha){$query->where('cad_entrada_saida.cod_cracha',$request->cod_cracha);}
		if($request->rel_posto){$query->where('cad_posto.id',$request->rel_posto);}
		if($request->rel_om){$query->where('cad_om.id',$request->rel_om);}
	...
	public function downloadMilitares(Request $request)
	{
		...
		if($request->cnh_cat){$query->where('cad_militar.cnh_cat','LIKE',"%{$request->cnh_cat}%");}
		...
		if($request->rel_posto){$query->where('cad_posto.id',$request->rel_posto);}
		if($request->rel_om){$query->where('cad_om.id',$request->rel_om);}
		if($request->status){$query->where('cad_militar.status',$request->status);}
		...
		foreach ($query as $key => $value) {
			...
			if($value->status == 1){$value->status = "Ativado";}else $value->status = "Desativado";
	...
	public function downloadAutomoveis(Request $request)
	{
		...
		if($request->placa_rel){$query->where('cad_automovel.placa','LIKE',"%{$request->placa_rel}%");}
		if($request->renavam){$query->where('cad_automovel.renavan',$request->renavam);}
		if($request->ano){$query->where('cad_automovel.ano_auto',$request->ano);}
		if($request->cor){$query->where('cad_automovel.cor','LIKE',"%{$request->cor}%");}
		if($request->rel_posto){$query->where('cad_posto.id',$request->rel_posto);}
		if($request->rel_om){$query->where('cad_om.id',$request->rel_om);}
		if($request->status){$query->where('cad_automovel.baixa',$request->status);}
		if($request->tipo_id){$query->where('cad_automovel.tipo_id',$request->tipo_id);}
		if($request->marca_id){$query->where('cad_automovel.marca_id',$request->marca_id);}
		if($request->modelo_id){$query->where('cad_automovel.modelo_id',$request->modelo_id);}
		...
		foreach ($query as $key => $value) {
			...
			if($value->baixa == 1){$value->baixa = "Desativado";}else $value->baixa = "Ativado";
