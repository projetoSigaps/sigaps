...
use App\Model\Sys\Cad_datas_detran;
...
class WebServicesController extends Controller
{
	...
	*/
	...
	public function pesquisaDataDetran(Request $request)
	{
		$placa = substr($request->placa, -1);

		$result = Cad_datas_detran::where('final_placa', $placa)->orderBy('id', 'desc')->first();

		if (!empty($result)) {
			return response()->json($result->dia_vencimento . "/" . str_pad($result->mes_vencimento, 2, '0', STR_PAD_LEFT) . "/");
		}
	}
